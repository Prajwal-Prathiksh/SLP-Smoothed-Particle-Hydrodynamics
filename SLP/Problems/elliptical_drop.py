...
class EllipticalDrop(Application):
    ...
    def create_scheme(self):
        ...
        dpsph = DeltaPlusScheme(
            fluids=['fluid'], solids=[], dim=2, rho0=self.ro, c0=self.co, 
            nu=0.0, p0=self.p0, hdx=0.0, dx=0.0, h0=0.0, dt=0.0,
        )
        s = SchemeChooser(default='dpsph', dpsph=dpsph, wcsph=wcsph, iisph=iisph)
        return s
    ...
    def configure_scheme(self):
        ...
        if self.options.scheme == 'wcsph':
            ...
            scheme.configure_solver(
                tf=tf, dt=dt, output_at_times=[0.0008, 0.0038]
            )
